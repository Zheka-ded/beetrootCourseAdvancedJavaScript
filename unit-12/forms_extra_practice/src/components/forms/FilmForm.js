...
class FilmForm extends Component {
    ...
    toggleSelect = e => this.setState( {select: e.target.value} )
        ...
        return (
            <form className="ui form" onSubmit={this.handleSubmit}>
                ...
                <div className="ui grid">
                        ...
                        <div className="grouped fields">
                            ...
                            {tags.map(tag => (
                                <div key={tag._id} className="field inline">
                                    <input type="checkbox" id={`tag-${tag._id}`} 
                                            checked={this.state.tags.includes(tag._id)}
                                            onChange={() => this.toggleTag(tag)} />
                                    <label htmlFor={`tg-${tag._id}`}>{tag.title}</label>
                                </div>
                            ))}
                        </div>
                    </div>
                        ...
                        <div className="field">
                            ...
                            {genres.map(genre => (
                                <div key={genre._id} className="field inline">
                                    <input type="radio" id={`genre-${genre._id}`}
                    ...
                    <div className="four wide column">
                        <label>Select</label>
                        <select value={this.state.select} onChange={this.toggleSelect} className="ui dropdown">
                            {genres.map(genre => 
